...
def load_algorithms(_algoritmi = None):
    ...
    if(_algoritmi == None):
        ...
        algoritmi = _algoritmi
    return algoritmi

def init_datasets(_target_variable_names = None):
    ...
    if(_target_variable_names == None):
        ...
        target_variable_names = _target_variable_names
    ...
    return preprocessed_datasets, target_variable_names

# Izracunaj ARR vrijednosti za svaki skup podataka, ovo je kombinacija metoda izracunaj_performanse i izracunaj_ARR
def compute_arr_values_for_dataset(X_train, y_train, algoritmi, alpha=0.05):
    
    perforanseAlg = izracunaj_performanse(X_train, y_train, algoritmi)

    arr_values = izracunaj_ARR(perforanseAlg, alpha)

    return arr_values

# Metoda za izracunavanje performansi algoritama za skup podataka
def izracunaj_performanse(X_train, y_train, algoritmi):
    results = []
    # Izracunaj preciznost i vrijeme izvršavanja za svaki algoritam
    for name, clf in algoritmi.items():
        scores = cross_validate(clf, X_train, y_train, scoring=['accuracy'], cv=10, return_train_score=True, n_jobs=1)
...
# Metoda za izracunavanje performansi algoritama za skup podataka
def izracunaj_performanse_norm(X_train, y_train, algoritmi):
    results = []
    # Izracunaj preciznost i vrijeme izvršavanja za svaki algoritam
...
def izracunaj_ARR(df_results, alpha=0.05):
    arr_values = []
...
def compute_metafeatures(X, y, _mfe = None):
    if(_mfe == None):
